#!/usr/bin/env node

/**
 * 🌊 SMART FLOOD 2026 — STANDALONE VIRTUAL ESP32 EDGE EMULATOR
 * ==============================================================================
 * A self-contained, disposable CLI client that emulates a physical ESP32 node.
 * Translates hydrological stage height into raw ultrasonic distance (cm),
 * streaming POST /api/v1/telemetry directly to localhost or the Render cloud backend.
 * ==============================================================================
 *
 * ⚠️  IMPORTANT DISCLAIMER — FOR DEVELOPMENT & TESTING USE ONLY
 * ==============================================================================
 * This file is a SOFTWARE DEVELOPMENT TOOL and is NOT part of the SmartFlood
 * 2026 capstone project documentation, manuscript, or system submission.
 *
 * - It does NOT represent the actual ESP32 microcontroller firmware.
 * - It is NOT included in the Technical Specification, Architecture Diagrams,
 *   or any chapter of the capstone paper.
 * - It exists solely to allow backend API testing and live dashboard
 *   demonstration without requiring physical hardware to be connected.
 * - The real embedded firmware is written in C++/Arduino IDE and resides
 *   separately in the microcontroller unit (not in this repository).
 *
 * DO NOT cite, reference, or submit this file as part of the project deliverable.
 * ==============================================================================
 */

import readline from 'readline';

// Default Target Endpoints
const RENDER_PROD_URL   = 'https://smart-flood2026-prototype.onrender.com/api/v1/telemetry';
const LOCALHOST_URL     = 'http://localhost:3001/api/v1/telemetry';

// Default to Render production; override with env var or toggle option [6]
let TARGET_URL = process.env.TELEMETRY_URL || RENDER_PROD_URL;

// Physical Hardware Constants (matches SmartFlood_ESP32.ino)
const SENSOR_MOUNT_HEIGHT_CM = 180; // 1.80m above riverbed

// System Uptime Tracker
let systemUptimeSec = 300;

// ANSI Terminal Colors
const C = {
  reset: '\x1b[0m',
  cyan: '\x1b[36m',
  green: '\x1b[32m',
  yellow: '\x1b[33m',
  red: '\x1b[31m',
  magenta: '\x1b[35m',
  bold: '\x1b[1m',
  dim: '\x1b[2m',
};

// Create CLI Interface
const rl = readline.createInterface({
  input: process.stdin,
  output: process.stdout,
});

const askQuestion = (query) => new Promise(resolve => rl.question(query, resolve));

/**
 * ESP32 Telemetry Packet Synthesizer
 * Converts stage height (m) into raw ultrasonic distance (cm)
 * with Gaussian surface acoustic jitter.
 */
function synthesizeTelemetryPacket(waterLevelM) {
  // Base raw distance in cm (mount height 180cm)
  const baseDistance = SENSOR_MOUNT_HEIGHT_CM - (waterLevelM * 100);

  // Gaussian surface acoustic chop jitter (±0.8 to 1.5 cm)
  const acousticJitter = (Math.random() - 0.5) * 2.2;
  let rawDistance = Math.round(baseDistance + acousticJitter);
  rawDistance = Math.max(20, Math.min(180, rawDistance));

  // Relay & Battery voltage dynamics (drops 0.3V when siren is energized)
  const relayActive = waterLevelM >= 1.4;
  const baseVoltage = 12.4 + (Math.random() * 0.3 - 0.15); // 12.4V ± 0.15V
  const batteryVoltage = parseFloat((relayActive ? baseVoltage - 0.3 : baseVoltage).toFixed(2));
  const wifiRssi = -58 + Math.floor(Math.random() * 9 - 4); // -58 ± 4 dBm

  systemUptimeSec += 10;

  return {
    rawDistance,
    batteryVoltage,
    wifiRssi,
    uptime: systemUptimeSec,
    relayState: relayActive,
  };
}

/**
 * Send POST /api/v1/telemetry packet to target server and parse response
 */
async function sendTelemetry(packet, stepStr = '01/01', comment = '', delaySec = 3.0) {
  try {
    const res = await fetch(TARGET_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(packet),
    });

    const isOk = res.ok;
    const statusText = `${res.status} OK`;
    let data = {};
    try {
      data = await res.json();
    } catch { /* silent */ }

    // Parse Response Highlights
    const logObj = data.log || data.data || {};
    const stageM = parseFloat(logObj.waterLevelM ?? logObj.water_level_m ?? (180 - packet.rawDistance) / 100).toFixed(2);
    const rawCm = logObj.rawDistanceCm ?? logObj.raw_distance_cm ?? packet.rawDistance;

    const alertStatus = data.event?.event_code || data.eventTriggered?.event_type || data.alertStatus?.eventCode || (stageM >= 1.6 ? 'ALERT_L3' : stageM >= 1.4 ? 'ALERT_L2' : stageM >= 1.0 ? 'ALERT_L1' : 'NORMAL');
    const aiEval = data.aiEvaluation || {};
    const rawConf = aiEval.avgAccuracy_pct;
    const ai30m = aiEval.forecast30min != null ? `${parseFloat(aiEval.forecast30min).toFixed(2)}m` : 'Evaluating...';
    const aiConf = typeof rawConf === 'number'
      ? `${rawConf.toFixed(1)}%`
      : (aiEval.confidenceScore ? `${aiEval.confidenceScore.toFixed(1)}%` : 'Evaluating...');
    const sirenText = (packet.relayState || data.sirenActive || stageM >= 1.4) ? `${C.red}${C.bold}ON 🚨${C.reset}` : `${C.dim}OFF${C.reset}`;

    let statusColor = C.green;
    if (alertStatus.includes('L1')) statusColor = C.yellow;
    if (alertStatus.includes('L2') || alertStatus.includes('L3')) statusColor = C.red;

    console.log(
      `[Step ${stepStr}] (${delaySec.toFixed(1)}s tick) POST -> ${isOk ? `${C.green}${statusText}${C.reset}` : `${C.red}${res.status} ${res.statusText}${C.reset}`} | ` +
      `Stage: ${C.bold}${stageM}m${C.reset} (Raw: ${rawCm}cm) | ` +
      `Status: ${statusColor}${alertStatus}${C.reset} | ` +
      `AI +30m: ${ai30m} (${aiConf}) | ` +
      `Siren: ${sirenText}` +
      (comment ? ` ${C.dim}— ${comment}${C.reset}` : '')
    );

    return { isOk, data };
  } catch (err) {
    console.log(`[Step ${stepStr}] ${C.red}POST FAILED${C.reset} -> ${err.message} (${TARGET_URL})`);
    return { isOk: false, error: err.message };
  }
}

/**
 * Delay Helper (ms)
 */
const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));

// ── 48-STEP TYPHOON HYDROLOGICAL SCENARIO DATASET ─────────────────────────────
const TYPHOON_48_STEPS = [
  // --- Phase 1: Dry Baseline (Steps 1-8, 5.0s interval) ---
  { step: 1,  stage: 0.35, delay: 5000, desc: "Dry Baseline (Calm)" },
  { step: 2,  stage: 0.35, delay: 5000, desc: "Dry Baseline (Buffer Warmup)" },
  { step: 3,  stage: 0.35, delay: 5000, desc: "Dry Baseline (Buffer Warmup)" },
  { step: 4,  stage: 0.35, delay: 5000, desc: "Dry Baseline (Buffer Warmup)" },
  { step: 5,  stage: 0.35, delay: 5000, desc: "Dry Baseline (Buffer Warmup)" },
  { step: 6,  stage: 0.35, delay: 5000, desc: "Dry Baseline (Buffer Ready)" },
  { step: 7,  stage: 0.35, delay: 5000, desc: "Dry Baseline (Stable Buffer)" },
  { step: 8,  stage: 0.36, delay: 5000, desc: "Pre-storm Inflow" },

  // --- Phase 2: Soil Saturation & Infiltration (Steps 9-16, 4.5s interval) ---
  { step: 9,  stage: 0.37, delay: 4500, desc: "Inflow Onset" },
  { step: 10, stage: 0.39, delay: 4500, desc: "Catchment Saturation Progressing" },
  { step: 11, stage: 0.42, delay: 4500, desc: "Moderate Inflow" },
  { step: 12, stage: 0.46, delay: 4500, desc: "Steady Stream Rise" },
  { step: 13, stage: 0.50, delay: 4500, desc: "Watershed Infiltration Limit" },
  { step: 14, stage: 0.55, delay: 4500, desc: "Surface Runoff Beginning" },
  { step: 15, stage: 0.60, delay: 4500, desc: "Runoff Feeding Tributaries" },
  { step: 16, stage: 0.66, delay: 4500, desc: "Stream Velocity Rising" },

  // --- Phase 3: Storm Surge & Threshold Breaches (Steps 17-24, 4.0s interval) ---
  { step: 17, stage: 0.74, delay: 4000, desc: "Torrential Inflow Surge" },
  { step: 18, stage: 0.85, delay: 4000, desc: "Rapid Inflow Surge" },
  { step: 19, stage: 0.98, delay: 4000, desc: "Approaching Level 1" },
  { step: 20, stage: 1.08, delay: 4000, desc: "BREACH: ALERT LEVEL 1 (Advisory)" },
  { step: 21, stage: 1.18, delay: 4000, desc: "Peak Inflow Surge" },
  { step: 22, stage: 1.28, delay: 4000, desc: "Surge Accelerating" },
  { step: 23, stage: 1.38, delay: 4000, desc: "Approaching Level 2 Alarm" },
  { step: 24, stage: 1.48, delay: 4000, desc: "BREACH: ALERT LEVEL 2 (Siren Active 🚨)" },

  // --- Phase 4: Peak Flash Flood Crest (Steps 25-28, 3.0s interval - Urgent) ---
  { step: 25, stage: 1.56, delay: 3000, desc: "Severe Flooding Surge" },
  { step: 26, stage: 1.62, delay: 3000, desc: "BREACH: ALERT LEVEL 3 (Peak Crest 🚨)" },
  { step: 27, stage: 1.60, delay: 3000, desc: "Hydrograph Peak Plateau" },
  { step: 28, stage: 1.55, delay: 3000, desc: "Crest Subsiding" },

  // --- Phase 5: Catchment Retention Lag (Steps 29-38, 5.0s interval) ---
  { step: 29, stage: 1.48, delay: 5000, desc: "Upstream Runoff Sustaining Stage" },
  { step: 30, stage: 1.42, delay: 5000, desc: "Inflow Ceasing, Channel Full" },
  { step: 31, stage: 1.35, delay: 5000, desc: "Water Level Dropping Below L2" },
  { step: 32, stage: 1.28, delay: 5000, desc: "Slow Soil Retention Drainage" },
  { step: 33, stage: 1.20, delay: 5000, desc: "Watershed Outflow Drain" },
  { step: 34, stage: 1.12, delay: 5000, desc: "Gradual Channel Drawdown" },
  { step: 35, stage: 1.05, delay: 5000, desc: "Approaching Level 1 Return" },
  { step: 36, stage: 0.98, delay: 5000, desc: "Water Level Below L1" },
  { step: 37, stage: 0.90, delay: 5000, desc: "Persistent Drainage Tail" },
  { step: 38, stage: 0.82, delay: 5000, desc: "Steady Baseflow Drainage" },

  // --- Phase 6: Drainage & Baseline Recovery (Steps 39-48, 6.0s interval) ---
  { step: 39, stage: 0.74, delay: 6000, desc: "Recession Limb Continuing" },
  { step: 40, stage: 0.66, delay: 6000, desc: "Low Flow Drainage" },
  { step: 41, stage: 0.58, delay: 6000, desc: "Recession Limb" },
  { step: 42, stage: 0.52, delay: 6000, desc: "Return to Near-Normal" },
  { step: 43, stage: 0.46, delay: 6000, desc: "Stream Settling" },
  { step: 44, stage: 0.42, delay: 6000, desc: "Channel Cleared" },
  { step: 45, stage: 0.38, delay: 6000, desc: "Approaching Baseflow" },
  { step: 46, stage: 0.36, delay: 6000, desc: "Residual Baseflow" },
  { step: 47, stage: 0.35, delay: 6000, desc: "Dry Channel Restored" },
  { step: 48, stage: 0.35, delay: 6000, desc: "Normal Baseline Complete" }
];

const SCENARIO_MODERATE_RAIN = [
  // Phase 1: Buffer Warmup (Steps 1-6)
  { step: 1,  stage: 0.35, delay: 5000, desc: "Normal Baseflow (Buffer Warmup)" },
  { step: 2,  stage: 0.35, delay: 5000, desc: "Normal Baseflow (Buffer Warmup)" },
  { step: 3,  stage: 0.35, delay: 5000, desc: "Normal Baseflow (Buffer Warmup)" },
  { step: 4,  stage: 0.35, delay: 5000, desc: "Normal Baseflow (Buffer Warmup)" },
  { step: 5,  stage: 0.35, delay: 5000, desc: "Normal Baseflow (Buffer Ready)" },
  { step: 6,  stage: 0.36, delay: 5000, desc: "Pre-storm Inflow Onset" },

  // Phase 2: Steady Inflow Onset (Steps 7-12)
  { step: 7,  stage: 0.38, delay: 4500, desc: "Light Steady Inflow" },
  { step: 8,  stage: 0.45, delay: 4500, desc: "Moderate Inflow Building" },
  { step: 9,  stage: 0.55, delay: 4500, desc: "Soil Saturation & Runoff" },
  { step: 10, stage: 0.68, delay: 4500, desc: "Stream Velocity Rising" },
  { step: 11, stage: 0.82, delay: 4500, desc: "Continuous Inflow Rise" },
  { step: 12, stage: 0.95, delay: 4500, desc: "Approaching Level 1 Watch" },

  // Phase 3: Level 1 Advisory Peak (Steps 13-16)
  { step: 13, stage: 1.05, delay: 4000, desc: "BREACH: ALERT LEVEL 1 (Advisory)" },
  { step: 14, stage: 1.15, delay: 4000, desc: "Peak Moderate Hydrograph (1.15m)" },
  { step: 15, stage: 1.12, delay: 4000, desc: "Inflow Easing - Water Plateau" },
  { step: 16, stage: 1.08, delay: 4000, desc: "Upstream Inflow Sustaining Stage" },

  // Phase 4: Drainage & Recovery (Steps 17-24)
  { step: 17, stage: 0.98, delay: 5000, desc: "Receding Below Level 1 Threshold" },
  { step: 18, stage: 0.88, delay: 5000, desc: "Inflow Ceasing, Channel Outflow" },
  { step: 19, stage: 0.75, delay: 5000, desc: "Steady Baseflow Drawdown" },
  { step: 20, stage: 0.62, delay: 5000, desc: "Recession Limb Progressing" },
  { step: 21, stage: 0.50, delay: 5000, desc: "Stream Channel Settling" },
  { step: 22, stage: 0.42, delay: 5000, desc: "Approaching Normal Baseflow" },
  { step: 23, stage: 0.36, delay: 5000, desc: "Residual Channel Flow" },
  { step: 24, stage: 0.35, delay: 5000, desc: "Normal Baseflow Restored (0.35m)" },
];

const SCENARIO_CALM_BASELINE = [
  { step: 1,  stage: 0.35, delay: 4000, desc: "Calm Baseflow (Normal 0.35m)" },
  { step: 2,  stage: 0.35, delay: 4000, desc: "Calm Baseflow (Ultrasonic Ripple)" },
  { step: 3,  stage: 0.36, delay: 4000, desc: "Calm Baseflow (Ultrasonic Ripple)" },
  { step: 4,  stage: 0.35, delay: 4000, desc: "Calm Baseflow (Ultrasonic Ripple)" },
  { step: 5,  stage: 0.35, delay: 4000, desc: "Calm Baseflow (Buffer Stable)" },
  { step: 6,  stage: 0.34, delay: 4000, desc: "Calm Baseflow (Ultrasonic Ripple)" },
  { step: 7,  stage: 0.35, delay: 4000, desc: "Calm Baseflow (Ultrasonic Ripple)" },
  { step: 8,  stage: 0.35, delay: 4000, desc: "Calm Baseflow (Ultrasonic Ripple)" },
  { step: 9,  stage: 0.36, delay: 4000, desc: "Calm Baseflow (Ultrasonic Ripple)" },
  { step: 10, stage: 0.35, delay: 4000, desc: "Calm Baseflow (Buffer Stable)" },
  { step: 11, stage: 0.35, delay: 4000, desc: "Calm Baseflow (Ultrasonic Ripple)" },
  { step: 12, stage: 0.35, delay: 4000, desc: "Calm Baseflow Baseline Complete" },
];

// ── MAIN CLI ROUTINE ──────────────────────────────────────────────────────────
async function main() {
  console.clear();
  console.log(`${C.cyan}${C.bold}`);
  console.log(`==================================================`);
  console.log(`   🌊 SMART FLOOD 2026 — VIRTUAL ESP32 EDGE NODE  `);
  console.log(`==================================================${C.reset}`);
  console.log(`Target URL: ${C.yellow}${TARGET_URL}${C.reset}\n`);

  console.log(`${C.bold}Select Mode:${C.reset}`);
  console.log(`[1] Scenario: Rapid Flash Flood Surge (Level 2 Crest -> Drain)`);
  console.log(`[2] Scenario: Moderate Stream Inflow (Level 1 Advisory)`);
  console.log(`[3] Scenario: Calm Dry Baseline with Ultrasonic Surface Ripples`);
  console.log(`[4] Continuous Real-Time Stream (Sends 1 packet every 5s indefinitely)`);
  console.log(`[5] Interactive Manual Mode (Enter custom water level)`);
  console.log(`[6] Toggle Target Server URL (currently: ${TARGET_URL.includes('localhost') ? C.yellow + 'Localhost' : C.green + 'Render Production'}${C.reset})`);
  console.log(`[Q] Quit`);
  console.log(`==================================================`);

  const choice = (await askQuestion(`Enter selection (1-6 / Q): `)).trim().toUpperCase();

  if (choice === 'Q') {
    console.log(`\nExiting Virtual ESP32 Node emulator. Goodbye!`);
    rl.close();
    process.exit(0);
  }

  if (choice === '6') {
    if (TARGET_URL.includes('localhost')) {
      TARGET_URL = RENDER_PROD_URL;
      console.log(`\n✅ Switched to ${C.green}Render Production${C.reset}: ${RENDER_PROD_URL}\n`);
    } else {
      TARGET_URL = LOCALHOST_URL;
      console.log(`\n✅ Switched to ${C.yellow}Localhost${C.reset}: ${LOCALHOST_URL}\n`);
    }
    await sleep(1500);
    return main();
  }

  if (choice === '1' || choice === '2' || choice === '3') {
    const dataset = choice === '1' ? TYPHOON_48_STEPS : choice === '2' ? SCENARIO_MODERATE_RAIN : SCENARIO_CALM_BASELINE;
    const title = choice === '1' ? '48-Step Authentic Typhoon Hydrological Cycle' : choice === '2' ? 'Moderate Stream Inflow' : 'Calm Dry Baseline';

    console.log(`\n🚀 Starting Scenario: ${C.bold}${title}${C.reset} (${dataset.length} steps)\n`);

    for (let i = 0; i < dataset.length; i++) {
      const step = dataset[i];
      const stageVal = step.stage != null ? step.stage : step.level;
      const descVal = step.desc != null ? step.desc : step.comment;
      const delayMs = step.delay != null ? step.delay : 3000;

      const packet = synthesizeTelemetryPacket(stageVal);
      const stepStr = `${String(i + 1).padStart(2, '0')}/${String(dataset.length).padStart(2, '0')}`;

      await sendTelemetry(packet, stepStr, descVal, delayMs / 1000);
      if (i < dataset.length - 1) await sleep(delayMs);
    }

    console.log(`\n✅ Scenario execution complete!\n`);
    const next = await askQuestion(`Press Enter to return to menu...`);
    return main();
  }

  if (choice === '4') {
    console.log(`\n🛰️  Starting Continuous Real-Time Stream (1 packet every 5s). Press Ctrl+C to stop.\n`);
    let stepCount = 1;
    let currentLevel = 0.35;

    while (true) {
      // Organic baseflow fluctuation ±0.02m
      currentLevel = Math.max(0.30, Math.min(1.80, currentLevel + (Math.random() * 0.04 - 0.02)));

      const packet = synthesizeTelemetryPacket(currentLevel);
      const stepStr = String(stepCount).padStart(3, '0');

      await sendTelemetry(packet, stepStr, 'Live Real-Time Stream');
      stepCount++;
      await sleep(5000);
    }
  }

  if (choice === '5') {
    console.log(`\n✍️  Interactive Manual Mode\n`);
    const lvlStr = await askQuestion(`Enter Water Level in meters (e.g. 1.25): `);

    const waterLevelM = parseFloat(lvlStr) || 0.35;

    const packet = synthesizeTelemetryPacket(waterLevelM);
    console.log(`\nSending Manual Telemetry Packet:`, packet);
    await sendTelemetry(packet, 'MANUAL', 'User Input');

    console.log(`\n✅ Packet transmitted!\n`);
    await askQuestion(`Press Enter to return to menu...`);
    return main();
  }

  console.log(`\nInvalid choice. Please select 1-6.`);
  await sleep(1500);
  return main();
}

main();
